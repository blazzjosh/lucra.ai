<script lang="ts">
	...
	import { AnimatedShinyText, BorderBeam } from '$lib/components/magic/index.js';
...
<section id="hero" class="relative mx-auto mt-32 max-w-7xl px-6 text-center md:px-8">
	<div class="backdrop-filter-[12px] group inline-flex h-7 -translate-y-4 animate-fade-in items-center justify-between gap-1 rounded-full border border-white/5 bg-white/10 px-3 text-xs text-white opacity-0 transition-all ease-in hover:cursor-pointer hover:bg-white/20 dark:text-black">
		<AnimatedShinyText class="inline-flex items-center justify-center">
			<span>✨ Introducing Lucra.ai</span>
			{' '}
			<Icons.ArrowRight
	...
	>
		Lucra is the new way
		<br class="hidden md:block" />
		{' '}
		to track and audit your finances.
	</h1>
	<p
	...
	>
	Lucra.ai is an AI-powered financial tracking and auditing tool that helps you
		<br class="hidden md:block" />
		{' '}
		 <a href="https://svelte.dev" class="underline underline-offset-2">Track your Expenses</a>, and
		<a href="https://animation-svelte.vercel.app" class="underline underline-offset-2">Audit your Accounts.</a>.
	</p>
	<Button
		href="/auth/login"
	...
	<div
		...
		class="relative mt-32 animate-fade-up opacity-0 [--animation-delay:400ms] [perspective:2000px] after:absolute after:inset-0 after:z-50 after:[background:linear-gradient(to_top,hsl(var(--background))_1%,transparent)]"
	>
		<div
