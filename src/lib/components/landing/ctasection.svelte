<script lang="ts">
	...
	import { Marquee } from '$lib/components/magic/index.js';
	...
	let tiles: { icon: any; bg: string }[] = [
		...
		{
			icon: Icons.ChartNoAxesColumnIncreasing,
		...
		<div class="flex w-full flex-col items-center justify-center">
			<div class="relative flex w-full flex-col items-center justify-center overflow-hidden">
				...
				<div class="absolute z-10">
					...
					>
						<Icons.HeartHandshake class="mx-auto size-16 text-black dark:text-white lg:size-24" />
					</div>
					<div class="z-10 mt-4 flex flex-col items-center text-center text-primary">
						<h1 class="text-3xl font-bold lg:text-4xl">Stop wasting time on boring bank statements.</h1>
						<p class="mt-2">Start your 7-day free trial. No credit card required.</p>
						<Button size="lg" variant="outline" href="/auth/login" class="group mt-4 rounded-[2rem] px-6">
							Get Started
							<Icons.ChevronRight
					...
					<div class="border bg-background absolute inset-0 -z-10 rounded-full opacity-40 blur-xl dark:bg-background"></div>
				</div>
				<div class="to-background absolute inset-x-0 bottom-0 h-full bg-gradient-to-b from-transparent to-70% dark:to-background"></div>
			</div>
		</div>
	</div>
</section>
